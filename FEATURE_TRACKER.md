...
- [x] Add setting to lock/unlock questions based on quiz progression in the Question Nav.
- [x] Ensure that new models can load in `Preferred Model` dropdown once an API key has been entered.
- [x] Ensure that appropriate models are removed from `Preferred Model` once the corresponding API key has been removed.
- [x] If no API key has been entered, the `Preferred Model` dropdown should not be clickable and should read `Connect an API key to see available models`.
- [x] Change `Preferred Model` option name to `Preferred Available Model`.
- [x] Remove the Settings `Quizzes` refresh button and implement auto-refresh.
- [x] Allow file deletion plus folder creation/deletion from the Settings quiz-folder manager.
- [x] Rename the Settings `Quizzes` box to `Quiz Folder Manager`.
- [x] Remove `Save Settings` and `Reload Settings` buttons, enable auto-save, and show save-status confirmation text.
- [x] Treat any saved OpenAI API key as sufficient to enable the `Preferred Available Model` dropdown, even if legacy auth-mode state remains.
- [x] Replace boxed Settings helper notes with plain inline note text.
- [x] Make `Quiz Folder Manager` folders collapsible and move deletion to a right-click context menu for quizzes and folders.
...
- [x] Fall back to cached or legacy-loaded provider models so `Preferred Available Model` still shows OpenAI options when preview-model loading is unavailable.
- [x] Add saved quiz clock mode and quiz timer duration settings that apply on quiz start/restart.
- [x] Add a Quiz Folder Manager deletion fallback plus clearer restart-required messaging when the running backend lacks delete-route support.
- [x] Move `Quiz Folder Manager` into its own main header tab.
- [x] Allow dragging a quiz onto another quiz inside a folder so the move targets that folder.
- [x] Highlight the full target folder area, including nested quizzes, while dragging quiz-folder-manager items.
- [x] Allow the Quiz Folder Manager list to be vertically resized by the user.
- [x] Ensure the Settings `New Folder` button handles not-ready managed-folder state cleanly and still creates folders in the selected location.
- [x] Make single-click folder selection non-collapsing; folders now collapse/expand only on double-click or the `+`/`-` toggles.
### Notes
...
- [x] Fix chart Date/Time axis labels and tooltips when using timestamp values.
- [x] Add Chart view with customizable axes.
- [x] Add a Performance History tab to the Quiz Navigation box.
- [x] Left-clicking a quiz opens performance history for that quiz by default.
- [x] Navigation arrows now navigate through quizzes' performance history.
- [x] Add filter by Stopwatch.
- [x] Add filter by Timer.
...
- [x] In Performance History `Sessions` view, `ArrowLeft` and `ArrowRight` now move to the quiz above and below in the visible quiz list.
- [x] In Performance History `Sessions` view, left/right quiz navigation no longer falls back to recent-history pages, and the header arrow buttons follow the same above/below quiz order.
...
- [x] Remove unused MCP OAuth setup section from `README.md`.
- [x] Refresh `README.md` to reflect current app features, MCP usage, and test/build commands.
- [x] 2026-03-07 bug-scan automation run: reran `pytest` + Electron build, reconfirmed 2 active bugs, and refreshed `BUGS_FOUND.md` with updated UI/edge-case test and regression plans.
- [x] 2026-03-07 bug-scan automation run: validated app status (`pytest`, Electron build), confirmed 2 active bugs, and refreshed `BUGS_FOUND.md` with UI/edge-case fix plan.
- [x] Add a weekly cleanup pass to archive completed items older than 30 days.
- [x] 2026-03-06 ideas automation refresh: rescanned app/tracker and rewrote `CODEX_IDEAS.md` with a new prioritized brainstorm set.
...
- [x] Keep `AGENTS.md` and `FEATURE_TRACKER.md` as local-only workflow files by untracking them and adding repo ignore rules.
- [x] Keep `/Quizzes` as a local-only library by untracking its contents and ignoring future additions.
